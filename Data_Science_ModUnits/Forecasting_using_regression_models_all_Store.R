...
########### Read data ###############
ds<-read.csv("C:/Users/dwoo57/Google Drive/Career/Data Mining Competitions/Kaggle/Walmart - Inventory and weather prediction/Cleaned_Data/v2_item_9_allstorefor_regressions_some_Stores_removed_allzeroes_rmv.csv",
             header = TRUE, sep = ",")
...
ds$store_nbr<-factor(ds$store_nbr)

str(ds)

#2 Partition Train and test data
train <-subset(ds, units !="NA")
nrow(train)
nrow(ds)

test <-subset(ds, is.na(units) )
nrow(test)
...
str(train)
fit <- randomForest(units ~ year_num + qtr + month_num + week_num_5wk + dayofweek_num + month_Avg_units +
                      store_nbr + 
...
write.table(test, file = "C:/Users/dwoo57/Documents/R/item9_allstore.csv", sep = ",", col.names = NA,
            qmethod = "double")
